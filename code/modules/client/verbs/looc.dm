...
/client/verb/looc(msg as text)
	...
	var/raw_msg = msg
	var/list/heard = get_hear(7, src.mob)
		...
		if(R_ADMIN)
			if(C.prefs.chat_toggles & CHAT_LOOC)
				...
				if (C.mob in heard)
					...
					to_chat(C,"[link]<font color='#6699CC'><span class='ooc'><span class='prefix'>(R)LOOC:</span> <EM>[display_name]:</EM> <span class='message'>[msg]</span></span></font>")
